...
class App:
    def __init__(self, master):
        self.master = master
        self.master.title("Camera Capture with HSV Filter")
        self.cap = cv2.VideoCapture(0)  # Initialize camera
        if not self.cap.isOpened():
            print("Error: Camera not found!")
            self.master.destroy()

        self.captured_image = None  # To store the last captured image
        self.hsv_image = None       # To store the HSV-converted image

        # Buttons
        self.open_btn = tk.Button(master, text="Open (Capture)", command=self.capture_image)
        self.open_btn.grid(row=0, column=0, columnspan=3)

        # Labels to display images
        self.original_img_lbl = tk.Label(master, text="Original Image")
        self.original_img_lbl.grid(row=1, column=0, columnspan=1)
        self.hsv_img_lbl = tk.Label(master, text="Filtered Image")
        self.hsv_img_lbl.grid(row=1, column=1, columnspan=1)

        # Sliders for HSV thresholds
        self.create_sliders(master)

    def create_sliders(self, master):
        # Hue sliders
        tk.Label(master, text="Hue").grid(row=2, column=0, pady=10)
        self.low_hue = self.create_slider(master, "Low", 0, 179, 0, 3, 0)
        self.high_hue = self.create_slider(master, "High", 0, 179, 179, 4, 0)

        # Saturation sliders
        tk.Label(master, text="Saturation").grid(row=2, column=1, pady=10)
        self.low_sat = self.create_slider(master, "Low", 0, 255, 100, 3, 1)
        self.high_sat = self.create_slider(master, "High", 0, 255, 255, 4, 1)

        # Value sliders
        tk.Label(master, text="Value").grid(row=2, column=2, pady=10)
        self.low_val = self.create_slider(master, "Low", 0, 255, 100, 3, 2)
        self.high_val = self.create_slider(master, "High", 0, 255, 255, 4, 2)

    def create_slider(self, master, label, min_val, max_val, default, row, column):
        slider = tk.Scale(
            master, from_=min_val, to=max_val, orient=tk.HORIZONTAL,
            length=200, command=self.update_filter, label=label
        )
        slider.set(default)
    ...
    def capture_image(self):
        ...
        self.captured_image = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)  # Convert BGR to RGB
        self.hsv_image = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)      # Convert BGR to HSV

        # Display the captured image
        self.update_original_image(self.captured_image)
        self.update_filter()

    def update_original_image(self, image):
        image = ImageTk.PhotoImage(Image.fromarray(image))
        self.original_img_lbl.configure(image=image)
        self.original_img_lbl.image = image
    ...
    def update_filter(self, *args):
        ...
        # Get slider values
        low_hsv = np.array([self.low_hue.get(), self.low_sat.get(), self.low_val.get()])
        high_hsv = np.array([self.high_hue.get(), self.high_sat.get(), self.high_val.get()])

        # Apply the filter
        mask = cv2.inRange(self.hsv_image, low_hsv, high_hsv)
        filtered = cv2.bitwise_and(self.captured_image, self.captured_image, mask=mask)

        # Display the filtered image
        filtered_image = ImageTk.PhotoImage(Image.fromarray(filtered))
        self.hsv_img_lbl.configure(image=filtered_image)
        self.hsv_img_lbl.image = filtered_image
